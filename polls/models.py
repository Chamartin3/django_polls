...
class Poll(models.Model):
    ...
    def expected_responses(self):
        return sumary.get('total')


    def responds_percentage(self):
        sumary = self.responses.sumary()
        return sumary.get('total')
        return participant in self.respondants()

    @property
    def poll_questions_responses(self):
        responses = self.responses.all()
        # import pdb; pdb.set_trace()
        questions=[]
        for quest in self.questions.all():
            quest.qresponses=[response for response in responses if response.question == quest ]
            # import pdb; pdb.set_trace()
    ...
    @property
    def expected_respondants(self):
        participants = self.cohort.participant.all()
        return participants

    def user_expected_respondants(self):
        return [part.user for part in self.expected_respondants]

    @property
    def expected_responses(self):
        return self.expected_respondants.count()
    ...
    def respondants(self):
        ...
        if questions:
            for i,q in enumerate(questions):
                if i<1:
                    responded = q.responded_by(self.id)
                else:
                    responded=[r for r in q.responded_by(self.id) if r in responded]
        return responded

    def user_respondants(self):
        return [part.user for part in self.respondants()]

    def to_response(self):
        return [x for x in self.expected_respondants if x not in self.respondants()]

    def get_genders(self):
        genders={
            'male': len([ x for x in self.expected_respondants if x.user.profile.gender == 'M']),
            'female': len([ x for x in self.expected_respondants if x.user.profile.gender == 'F']),
            'unknown': len([ x for x in self.expected_respondants if x.user.profile.gender == 'U'])
        }
        return genders

    def age_dist(self):
        return frecuency_dist(self.expected_respondants)

    @property
    def total_responses(self):
        return len(self.respondants())

    @property
    def responds_percentage(self):
        return (self.total_responses / self.expected_responses) *100

    @property
    def responds_percentage(self):
        ages = [resp for resp in self.expected_respondants. ]